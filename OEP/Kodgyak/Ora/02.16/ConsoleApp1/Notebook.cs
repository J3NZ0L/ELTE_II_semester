...
public class Notebook
{
	...
	private readonly Sort sort; // tipus
	private readonly List<string> pages; // lapok
	private int empty; // ures
	public Sort NotebookSort { get { return sort; } }
	...
	public string this[int idx]
	{
		get
		{
			--idx;
			if (idx < 0 || idx >= pages.Count)
				throw new InvalidPageException(message: $"Page {idx + 1} does not exist.");
			return pages[idx];
	...
	public void WriteOnPage(int ind, string content)
	{

	}
	public void TearOut(int ind)
	{
		--ind;
		if (ind < 0 || ind >= pages.Count)
	...
	///this is a ValueTuple under the hood
	public (bool found, int ind ) Search()
	{
		//TODO
	}
